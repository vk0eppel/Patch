...
2. **Network interface** — usually nothing to do here: if your machine has one usable network, Patch pins to it automatically. If it has more than one, pick one in **Settings → Network Interface** (a pinned interface ignores peers on other networks; static peers stay reachable); changes apply within a few seconds, no restart.
