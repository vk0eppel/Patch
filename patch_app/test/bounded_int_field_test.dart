...
void main() {
  ...
  testWidgets('a valid value is submitted', (tester) async {
    ...
    await tester.pumpWidget(
      _host(value: 7, min: 1, max: 60, onSubmit: (v) => submitted = v),
    );
  ...
  testWidgets('boundaries are accepted', (tester) async {
    final got = <int>[];
    await tester.pumpWidget(
      _host(value: 5000, min: 1024, max: 65535, onSubmit: got.add),
    );
  ...
  testWidgets(
    'an out-of-range value shows the range error and is not submitted',
    (tester) async {
      int? submitted;
      await tester.pumpWidget(
        _host(
  ...
  testWidgets('a non-numeric value shows an error and is not submitted', (
    tester,
  ) async {
    int? submitted;
    await tester.pumpWidget(
  ...
  testWidgets(
    'a valid value is committed on focus-loss, without pressing Enter',
    (tester) async {
      int? submitted;
      await tester.pumpWidget(
        _host(value: 7, min: 1, max: 60, onSubmit: (v) => submitted = v),
  ...
  testWidgets(
    'focus-loss with an unchanged valid value does not re-submit spuriously',
    (tester) async {
      final got = <int>[];
      await tester.pumpWidget(
        _host(value: 7, min: 1, max: 60, onSubmit: got.add),
      );
      await tester.tap(find.byKey(const Key('elsewhere')));
      await tester.pump();
