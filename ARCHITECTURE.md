...
└── patch_app/lib/
    ...
    ├── presenters/home_controller.dart  # Home store→presenter wiring + one-shot gates
...
**`HomePresenter`** (`presenters/home_presenter.dart`) owns the flash state machine and all config-derived flash settings. It self-subscribes to a `Stream<AppConfig?>` passed at construction (`configStream` — fed by `HomeController`, `presenters/home_controller.dart`, which owns the store→stream fan-out plus the one-shot gates: the first-run name prompt, the first-load macros-panel default, and channel-set change detection for selection reconcile) and applies settings changes via `_applyConfig`; `home_screen.dart` never calls imperative setters on the presenter for config — it applies the `HomeStoreEffects` the controller returns. `HomePresenter` also owns DM peer lookup: it subscribes to a `Stream<List<PeerInfo>>` (`peersStream`) and exposes `dmPeerName(peerId)` and `isDmPeerOffline(peerId)` — `home_screen.dart` delegates both lookups to the presenter rather than maintaining its own peer-list copy.
