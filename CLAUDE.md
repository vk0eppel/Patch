...
└── patch_app/                       # Flutter UI
    ...
    └── lib/
        ...
        └── widgets/
            ...
            └── peers_panel.dart     # Left panel (toggle, beside the channel strip) — online peers, discovery mode
```
...
send_osc_macro(address, port, path, arg: Option<String>) -> Result<()>  // fire an OSC packet to external gear (dual-action macro)
send_direct_message(peer_id, payload, priority) -> Result<String>       // private 1:1 message, unicast to one peer; stored under dm:{peer}
send_dm_flash(peer_id) -> Result<()>                                    // direct flash/attention ping, unicast to one peer; flashes the recipient's DM thread
...
upsert_macro(channel_id, label, payload, priority, key_binding, midi_note: Option<u8>, midi_cc: Option<u8>, osc: Option<OscTarget>) / delete_macro(channel_id, label)
reorder_macros(channel_id, ordered_labels)                       // drag-to-reorder; unlisted labels kept, unknown ignored
upsert_global_macro(label, payload, priority, key_binding, midi_note: Option<u8>, midi_cc: Option<u8>, osc: Option<OscTarget>) / delete_global_macro(label)  // shown on every channel
...
/patch/channel/{id}/say         # Simple external-OSC post (payload + optional priority); receiving node originates+relays it
...
/patch/dm                       # Direct (1:1) message — unicast; args incl. target_id; stored under dm:{sender}
/patch/dm/flash                 # Direct flash/attention ping — unicast; args: sender_id, sender_name, target_id; flashes dm:{sender}
...
**OSC target (dual action)**: a macro can also carry an optional `OscTarget` (`address`/`port`/`path`/`arg`,
`state/channel.rs`). When it fires (tap, F-key, **or MIDI**) Patch sends the normal Patch message to peers
**and** an OSC packet to the target — e.g. trigger a QLab cue or Companion button from the same callout
button. Encoded by `osc::codec::encode_osc` and sent via `api::dispatch_osc` (the FFI `send_osc_macro` for
the UI's `_fireMacro`; `midi::resolve_osc`/`fire` for MIDI-fired macros — once per matched macro). Edited via
the "Also send OSC" toggle in the macro editor; shown as an **OSC** badge on the macro row. (Inbound — mapping
foreign OSC → Patch messages — is still a separate TODO.)

**MIDI triggers**: the engine's `midi` module (desktop-only — CoreMIDI/WinMM/ALSA via `midir`; no-op on
iOS/Android) opens every input port at startup and fires a macro hands-free on a bound Note On (velocity > 0)
or CC (value ≥ 64). Routing mirrors `_fireMacro`: a **per-channel** macro fires on **its own channel**
(absolute, engine-side — unlike F-keys, so it works without focus); a **global** macro fires on the
**currently-selected channel(s)** — the engine learns the selection because Flutter pushes it via
`set_selected_channels` on every selection change (it includes `__all__` in ALL mode, so a global macro fired
then broadcasts, exactly like a tap). See the dev note below for the threading model + the pure
`resolve_targets` routing.
...
- **Direct messages (DMs):** private 1:1, on a **dedicated `/patch/dm` packet** (not a channel message — a `dm:{uuid}:{uuid}` channel_id would fail `valid_channel_id`). The packet carries `target_id` (recipient peer id); `api::send_direct_message` unicasts **only** to that peer and stores a local copy keyed `dm:{target_id}`. `decode_dm` sets the received message's `channel_id = dm:{sender_id}`, so **each side keys the thread by the other peer** (the `dm:` key never goes on the wire, so the slug rule doesn't apply). `transport::handle_event`'s `DirectMessage` arm checks `target_id == client_id`, auto-registers the sender, and `store_message`s it. Best-effort: no retransmit/ACK for critical DMs (unlike channel criticals). UI (`home_screen`): DM mode is an **exclusive selection** like ALL (`_isDmMode`/`_dmPeerId`); threads live in `_openDms` (opened) ∪ `_messages` keys starting with `dm:`; a 💬 `_DmTab` per thread in the sidebar with an unread dot (`_unreadDms`); DMs are **excluded from the ALL feed and from `set_selected_channels`** (so a MIDI global macro never fires on a `dm:` key); an inbound **non-critical DM message** doesn't flash (silent unread dot); a **critical DM** flashes via `_triggerDmFlash` (gated on the global `_flashOnCritical` flag, default on — so it follows the same opt-out as channel criticals). clear/export scope to the one thread. The flash + macros header buttons stay visible in DM mode, and **firing a macro in a DM sends its text as a DM** (`_fireMacro` DM branch — quick canned replies; only global macros show since no channel is selected). **Direct flash (attention ping):** the header flash button in DM mode calls `send_dm_flash(peer_id)` — a dedicated `/patch/dm/flash` packet (args: sender_id, sender_name, target_id), unicast **only** to that peer (`encode_dm_flash` / `PatchEvent::DirectFlash`). The receiver's `transport::handle_event` `DirectFlash` arm checks `target_id == client_id`, auto-registers the sender, and publishes `AppEvent::ChannelFlash` with `channel_id = dm:{sender_id}` (built locally, so it skips `valid_channel_id` which rejects `dm:` keys). The sender publishes the same locally keyed `dm:{target}` so it sees its own ping. UI: `channel_flash` events with a `dm:` id route to `_triggerDmFlash` — distinct from a plain DM, it **plays the alert sound**, opens the thread's tab (`_openDms`), pulses the message area when in view, else sets the unread dot. Best-effort (no ACK/retransmit), mirroring `send_direct_message`. **Offline warning:** because DMs have no delivery receipt, after *any* DM send (typed, macro, or flash) the UI calls `_warnIfDmPeerOffline` — if the target peer has no resolved address or hasn't been heard from within the peers-panel offline window (`> 5× heartbeat`, the grey-dot threshold via `_isDmPeerOffline`), it raises an amber SnackBar ("… appears offline — they may not receive this DM"). The message is still stored locally and sent best-effort; the warning is advisory. `_ChannelView` (stateless) doesn't own the peer list/context, so it fires the parent's `onDmSent` callback after a DM send and `_HomeScreenState` decides whether to warn. The channel-reload prune keeps `dm:` ids (like `__all__`). DM button (`peers_panel.dart` `onDm`) is shown only for **real** peers — not synthetic `ManualIp` entries (their derived id can't reach a live node).
- **Inbound `/say` (external OSC injection, e.g. QLab):** `decode_say` (codec) parses `/patch/channel/{id}/say` → `PatchEvent::Say { channel_id, payload, priority }` — arg 0 = payload (string), arg 1 = optional priority (int/long/float/double, lenient: missing or out-of-range → Info so a fat-fingered cue still posts). `transport::handle_event`'s `Say` arm has the receiving node **originate** the message (its own `client_id`/`client_name`, a fresh `message_id` + `now`), **relay** it to every known peer (inline dedup-by-addr loop, mirroring `send_to_peers`; criticals are `reliability.track`ed for retransmit), and `store_message` it locally — so an OSC source posts to the **whole crew** through one node, exactly as if typed there. No Dart/FRB change (rides the normal `message` event). Validated like `/message` (`valid_channel_id` + `MAX_PAYLOAD_LEN`). The arbitrary-foreign-address → message *mapping table* is still a separate TODO; `/say` covers controllable senders (QLab/Companion/scripts).
...
- **MIDI input** (`patch-core/src/midi/mod.rs`, `crate::midi::start` called from `api::init`): desktop-only via `midir` (declared under `[target.'cfg(any(macos, windows, linux))'.dependencies]`; the module has a `#[cfg(...)]` real `backend` and a `#[cfg(not(...))]` no-op `backend`, so iOS/Android build with **no** midir dep). Opens **every** physical input port at startup **plus** (macOS/Linux only) a **virtual input port "Patch"** (`MidiInput::create_virtual` via `midir::os::unix::VirtualInput`, in `connect_all`) so other software/gear can send MIDI to Patch with no hardware — it appears as a "Patch" MIDI destination and feeds the same pipeline; Windows (WinMM) has no virtual ports, so those users route through a loopback (loopMIDI) + a physical port. The OS callback parses Note On (vel > 0) / CC (value ≥ 64) and forwards a `MidiTrigger` over a tokio `unbounded_channel` to a task that runs the pure **`resolve_targets`** routing (unit-tested): **per-channel** macros fire on **their own channel** (absolute); **global** macros fire on **each currently-selected channel** — Flutter pushes the selection to `AppState.selected` via `api::set_selected_channels` on every selection change (incl. the channels handler), and `__all__` is included in ALL mode so a global macro fired then broadcasts (mirrors the UI's `_fireMacro` exactly). The `MidiInputConnection`s are kept alive for the process lifetime by a dedicated parked `std::thread` (dropping a connection closes its callback — same pattern as the mDNS daemon handle), so nothing is stored on `EngineHandle`. Firing goes through `api::dispatch_message` — the send path **extracted** from `send_message` (takes `&AppState`, `&Arc<Transport>`, `&Arc<Mutex<ReliabilityManager>>`) so MIDI-fired criticals are ACK-tracked and the local flash/alert fires exactly like a hand-sent message. Per-channel MIDI is **absolute/engine-side** (fires regardless of focus) — deliberately different from F-keys (`home_screen._handleHardwareKey`, Flutter-side). CC fires on ≥ 64 with no edge-tracking, so a momentary footswitch (127→0) fires once; an expression pedal would spam (don't bind one). Ports are enumerated once at startup — hot-plug needs a restart (rescan is a possible follow-up). `api::get_midi_ports()` exposes the port names for a future selector UI (no UI yet). CI installs `libasound2-dev` (the ALSA `-sys` headers) for the Linux build. **macOS framework linking (load-bearing):** `midir`/`coremidi-sys` emit `cargo:rustc-link-lib=framework=CoreMIDI`/`CoreAudio`, but those directives are **lost** when cargokit produces a *static* `libpatch_core.a` that Xcode links (a `.a` doesn't carry them) — so the macOS app fails to link with undefined `_MIDIClientCreate` / `_AudioGetCurrentHostTime` symbols. Fix is in `patch_app/rust_builder/macos/patch_core.podspec`: `OTHER_LDFLAGS` appends `-framework CoreMIDI -framework CoreAudio` (run `pod install` in `patch_app/macos` after editing the podspec so the generated `Pods/Target Support Files/patch_core/*.xcconfig` picks it up). **iOS needs nothing** — `midir` is excluded there by the `cfg`. If you ever add another Rust crate that links a system framework, it'll need the same podspec treatment.
...
- **ALL channel (crew-wide broadcast)**: a pinned **ALL** tab at the top of the channel strip (reuses `ChannelTab` with a synthetic channel coloured `PatchTheme.broadcast` — **neutral soft light-grey** (`#CCCCCC`), deliberately not a real colour: ALL isn't a department and the old accent blue clashed with RF — id `kAllChannelId = '__all__'`, defined in `models/message.dart`). It's an **exclusive** selection — tapping ALL sets `_selectedIds = {'__all__'}`; tapping a normal channel exits ALL mode (`_toggleChannel`). Sending in ALL mode emits **one** message on `__all__` (`send_message('__all__', …)` — no new OSC address, no FRB regen); because a channel id is just a routing label (every peer receives every packet) and the UI shows `__all__` messages in **every** feed, it reaches every peer regardless of their channels, *including channels the sender doesn't have*. Read side: ALL mode merges all `_messages` keys; normal mode folds `_messages['__all__']` into each channel feed so a broadcast shows wherever you're looking. Broadcast rows render a 📢 marker (`message_list.dart`), the input shows a broadcast hint, and `_triggerBroadcastFlash` pulses the ALL tab + message area in the neutral broadcast grey (`PatchTheme.broadcast`). Delivery badge / critical retransmit / global macros work unchanged (a broadcast critical is just a critical). The engine reserves `__all__` (`api::upsert_channel` rejects it). Two gotchas handled: the channel-reload stale-id prune keeps `__all__` (else a reload kicks you out of ALL mode), and `_fireMacro`/header/send/clear/export all special-case ALL since `selectedChannels` is empty in that mode.
...
- **peers panel**: 160 px wide (`_kPeersPanelWidth`); sits on the **left**, between the channel strip and the message area (toggled by the `people` icon at the **left** of the `_ChannelView` header, mirroring the panel; the macros toggle stays on the right), with a left `Border` separating it from the same-coloured strip — "who/where" context (channels + peers) is grouped left, macros stay right; header is "PEERS" (not "ONLINE"); 3-state dot — green ≤ 2× heartbeat, amber ≤ 5× (heartbeat missed / going quiet), gray beyond or ManualIp (configured-only); thresholds derive from `heartbeat_interval_secs` (default 7 s → 14 s / 35 s); each row's subtitle leads with the **IP then the "last seen" time** ("192.168.1.5 · 30s ago") for dynamic peers — the dot already conveys liveness, so the stable IP leads and the changing timer trails — or just the address for ManualIp; each row also shows **channel-membership dots** (one per channel the peer announces in `PeerInfo.channels`, coloured from the viewer's own channel map — grey for a channel the viewer lacks — capped at 5 then "+N"; `home_screen` passes `PeersPanel.channelColors`) and, when set, a **neutral role chip** (`_RoleBadge`, plain text, no colour) after the name; peers persist for the full session and never auto-expire; static peers always appear even before first contact (gray dot with 📌 icon); `PeersPanel` is a `StatefulWidget` with a `Timer.periodic(3s)` that calls `setState` to keep dot colours and the relative-time counter current without waiting for an external event; `person_remove_outlined` button in the panel header calls `onClearStale` → `clearStalePeers(maxAgeSecs: 60)` — removes OscBeacon/Mdns peers not heard from in 60 s; ManualIp/static peers are never removed; `PeerExpired` is emitted per removed peer so the panel refreshes automatically
