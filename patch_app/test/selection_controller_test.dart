...
void main() {
  group('selectTab', () {
    test(
      'tapping the ALL sentinel from a ChannelSelection stashes the current ids',
      () {
        final c = _ctrl();
        c.selectTab('rf');
        c.selectTab('audio');
    ...
    test(
      'tapping a dm:-prefixed id produces DmSelection regardless of prior state',
      () {
        final c = _ctrl();
        c.selectTab('rf');
    ...
    test(
      'tapping a plain id from AllSelection cancels ALL and selects just that channel',
      () {
        final c = _ctrl();
        c.selectTab('rf');
    ...
    test(
      'tapping a plain id from DmSelection cancels DM mode and selects just that channel',
      () {
        final c = _ctrl();
        c.selectTab('dm:p1');
        final toFetch = c.selectTab('audio');
    ...
    test(
      'toggling an already-selected id out of a multi-channel selection removes it',
      () {
        final c = _ctrl();
        c.selectTab('rf');
        c.selectTab('audio');
    ...
    test(
      'toggling the last remaining selected id is a no-op — never an empty selection',
      () {
        final c = _ctrl();
        c.selectTab('rf');
        final toFetch = c.selectTab('rf');
  ...
  group('openDm', () {
    test(
      'always produces DmSelection(peerId), regardless of prior selection',
      () {
        final c = _ctrl();
        c.selectTab('rf');
        c.selectTab('audio');
  ...
  group('snapBackFromAll', () {
    ...
    test(
      'leaves the selection untouched when both previous and channels are empty',
      () {
        final c = _ctrl();
        c.selectTab(kAllChannelId);
  ...
  group('reconcileWithChannels', () {
    test(
      'drops stale ids from a ChannelSelection, keeping ids still present',
      () {
        final c = _ctrl();
        c.selectTab('rf');
        c.selectTab('audio');
    ...
    test(
      'reseeds to the first channel when no previously-selected id survives',
      () {
        final c = _ctrl();
        c.selectTab('rf');
        final toFetch = c.reconcileWithChannels([_ch('audio'), _ch('lx')]);
    ...
    test(
      'reseeds to an empty ChannelSelection when the channel list itself is empty',
      () {
        final c = _ctrl();
        c.selectTab('rf');
  ...
  group('side effects — ensureMessages', () {
    test(
      'selectTab(channelId) calls ensureMessages for the selected channel',
      () {
    ...
    test(
      'selectTab(kAllChannelId) calls ensureMessages for the ALL sentinel',
      () {
        final store = _FakeStore();
        final c = SelectionController(store, FakeBridge());
    ...
    test(
      'snapBackFromAll calls ensureMessages for the restored channel ids',
      () {
        final store = _FakeStore();
        final c = SelectionController(store, FakeBridge());
        c.selectTab('rf');
    ...
    test(
      'reconcileWithChannels calls ensureMessages for the surviving channel',
      () {
        final store = _FakeStore();
        final c = SelectionController(store, FakeBridge());
        c.selectTab('rf');
  ...
  group('side effects — syncSelection', () {
    test(
      'selectTab(channelId) pushes channelId to setSelectedChannels and null to setDmTarget',
      () {
    ...
    test(
      'selectTab(dm:peer) pushes empty channel list and peerId to setDmTarget',
      () {
        final bridge = FakeBridge();
        final c = SelectionController(_FakeStore(), bridge);
    ...
    test(
      'selectTab(kAllChannelId) pushes the ALL sentinel to setSelectedChannels and null DM target',
      () {
        final bridge = FakeBridge();
        final c = SelectionController(_FakeStore(), bridge);
