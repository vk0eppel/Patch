...
│   ├── dm.rs               # DmThreadKey — owns the dm:{peer_id} thread-key invariant
│   ├── macro_router.rs     # Macro routing for every trigger source (ADR-0009)
...
└── patch_app/lib/
    ...
    │                              # typed fromRust views over the FRB structs,
    │                              # the one inbound construction path (see CONVENTION.md)
    ├── presenters/home_presenter.dart   # Flash state, store reduction + one-shot gates, peer lookup
    ├── presenters/settings/             # Per-section presenters: validate→save→refetch loops
    ├── screens/home_screen.dart   # Main UI
    ├── screens/settings_screen.dart     # Composition + import flows; sections live in widgets/settings/
    ├── widgets/                   # channel_tab, flash_button, message_list,
    │                              # macros_panel, peers_panel, show_files_dialog
    └── widgets/settings/          # Settings section widgets (identity, network,
                                   # behavior, static peers, macros)
```
...
**`messaging.rs`** is the shared origination layer. `originate_channel_message` is the one channel-message pipeline — encode → fan-out (`Delivery::Direct` sends via the transport; `Delivery::Relay` returns packets for `transport::receive_loop` to emit) → Critical tracking → local store → zero-reachable-peers failure report (#132, coded once). `dispatch_channel_message` and `originate_for_relay` are thin wrappers over it. Flash origination lives here too (`dispatch_flash`, `dispatch_dm_flash`), as does `dispatch_dm` (direct message sends via FFI and `macro_router`) and `send_to_peer_by_id` — the single-target request orchestration (peer lookup → `best_addr` → encode → unicast, shared by `request_channels`/`request_global_macros`). `api.rs` is a thin FFI facade: each `pub` function parses its string/int arguments, calls the appropriate `messaging::*`/`macro_router::*` function, and returns. `macro_router`'s DM branch calls `messaging::dispatch_dm` directly using the injected `state`/`transport` — it never re-enters `engine()`.
...
The Settings screen is composition only: each section (identity, network, behavior, static peers, global macros, channels & macros) is a widget in `widgets/settings/`, fronted by a presenter in `presenters/settings/` that owns the section's validate→bridge-call→refetch loop with bridge/store dependencies injected as functions — the loops are unit-tested without FFI. Bounds and input validation happen in the presenter *before* any bridge call; the widgets keep only presentation (controllers, saved ticks, dialogs). `settings_screen.dart` retains the scaffold/rail/scrollspy and the cross-section peer-import flows.
...
/patch/macros/request           ask a peer for its global macros
/patch/macros/announce          reply with global macros JSON
```
...
**`HomePresenter`** (`presenters/home_presenter.dart`) owns the flash state machine and all config-derived flash settings. The Home screen's store listener calls `onStoreChanged` directly (#160 — no intermediate controller hop): the presenter applies config/peers and returns `HomeStoreEffects` deciding the one-shot gates (the first-run name prompt, the first-load macros-panel default, and channel-set change detection for selection reconcile); `home_screen.dart` never calls imperative setters on the presenter for config — it applies the returned effects. (An optional `configStream` constructor parameter remains as a test seam.) `HomePresenter` also owns DM peer lookup: it subscribes to a `Stream<List<PeerInfo>>` (`peersStream`) and exposes `dmPeerName(peerId)` and `isDmPeerOffline(peerId)` — `home_screen.dart` delegates both lookups to the presenter rather than maintaining its own peer-list copy.
...
**mDNS liveness rule:** a `PeerSighting::Mdns` sighting sets address+port but never updates `last_seen`. New mDNS-only peers are inserted already-stale (backdated 60 s). Reason: `mdns-sd` replays cached resolutions for ~1–2 min after a peer quits, so bumping `last_seen` there kept departed peers green past the heartbeat window and undid `/patch/bye` expiry. Note: the per-address timestamp (used by the 3× prune) is always stamped with `Utc::now()` at resolution time — not with `peer.last_seen`. These are independent clocks: `last_seen` tracks OSC liveness; the address timestamp tracks route freshness. Using `peer.last_seen` for the address timestamp caused immediate eviction for Offline peers on re-resolution (see ERRORS.md).
...
DMs use a dedicated `/patch/dm` packet — not a channel message (`dm:` prefixed IDs fail `valid_channel_id`). Thread keying: sender stores under `dm:{target_id}`; receiver stores under `dm:{sender_id}`. Each side keys the thread by the other peer. The `dm:` prefix never goes on the wire. The key shape is owned by one type per language — `DmThreadKey` (`patch-core/src/dm.rs`) and `DmThread` (`lib/models/dm_thread.dart`); no raw `"dm:"` literals exist outside them.

DMs are excluded from the ALL feed and `set_selected_channels`. Non-critical DMs produce a silent unread dot on the peer row (`HomePresenter.markDmUnread`); critical DMs flash via `HomePresenter._handleFlashPush` → `reduceEvent` → `decideMessageFlash`. Firing a macro in DM mode sends its text as a DM — routing is engine-owned for every trigger source (tap/F-key via the `fire_macro`/`fire_key_binding` FFI commands, MIDI via `fire_trigger`; all through `macro_router`, keyed off `AppState::dm_target` — ADR-0009). No ACK/retransmit for DMs (best-effort).
...
Routing is handled by `crate::macro_router` — the single dispatch layer for **every** trigger source (ADR-0009). `midi::backend::MidiTrigger` implements `macro_router::MacroTrigger` (one method: `matches(&MacroMessage) -> bool`) and calls `macro_router::fire_trigger`; a UI tap goes through the `fire_macro(channel_id?, label)` FFI command and an F-key through `fire_key_binding(label)` — both funnel into the same `fire_matching` core (`fire_identified` narrows the macro universe to the one identified macro). The router checks DM mode first: if a DM thread is open (`AppState::dm_target`, pushed from Flutter via `set_dm_target` alongside `set_selected_channels`), every matched macro — per-channel or global alike — sends as a DM (`resolve_dm_payloads`), channel-independent. Otherwise `resolve_targets` applies: per-channel macros fire on their own channel (absolute, engine-side); global macros fire on currently-selected channels (Flutter pushes selection via `set_selected_channels`). F-key precedence (a per-channel macro on a selected channel beats a global on the same key; unselected channels' bindings never fire) is `resolve_key_macro`, engine-side; the Dart key handler keeps only a synchronous is-anything-bound existence check to decide event consumption.
...
Channel IDs are slugs (`[a-z0-9_-]`, ≤64 chars). `osc::codec::valid_channel_id` is the wire-level check (used for routing a Message/Flash/Say's `channel_id`, where `__all__` must still decode as a legal broadcast target). `channel::validate_channel_id` is the stricter "is this a legal Channel" check — the same slug rule *plus* rejecting `__all__` as reserved — and is the single source of truth for that question: `Channel::new` calls it for the one path that constructs a `Channel` from a raw id (`api::upsert_channel`'s FFI argument), and every untrusted-input path that validates a `Channel` already built by `serde::Deserialize` (a show file, a peer's `channels_announce`) calls it directly. Default channels ship macro-less; common callouts are seeded as global macros shown on every channel. Global macros fire on currently-selected channel(s). `channel::validate_binding_unique` is the single source of truth for "does this macro's key/MIDI binding collide with another's" — checked across three tiers (global-vs-global, a channel's macros against that same channel's other macros, and a per-channel macro against every global macro and vice versa) at every macro entry point: live create/edit in Settings, Show File load, `patch.toml` load, and peer macro import. A binding collision is rejected outright on live create/edit; load/peer paths drop just the colliding binding and keep the macro (ADR-0002/ADR-0006's drop-and-warn posture). A macro's OSC dual-action target is additionally pre-checked Dart-side by `validateMacroOscTarget` (`presenters/settings/macro_osc_validator.dart`) so a live edit rejects at the Save button; the engine's `validate_osc_target` remains the authority. This replaces what used to be undocumented and unchecked: two macros could previously share an F-key/MIDI binding with no warning, and the per-channel macro would win at fire time on a shared F-key. `macros_panel.dart` exports both `MacrosPanel` and `ChannelMacro`.
