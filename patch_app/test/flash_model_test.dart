...
void main() {
  ...
  group('reduceEvent — MessageReceived', () {
    test(
      'flash-eligible message on selected channel bumps flashCounts and flashNotify',
      () {
    ...
    test(
      'flash-eligible message on unselected channel bumps flashCounts only',
      () {
    ...
    test(
      'non-critical DM not selected marks unread and bumps dmPulseNotify when panel closed',
      () {
        final sel = ChannelSelection({'rf'});
        final s = reduceEvent(
          FlashState.empty,
    ...
    test(
      'non-critical DM not selected, peers panel open — marks unread, dmPulseNotify unchanged',
      () {
        final sel = ChannelSelection({'rf'});
        final s = reduceEvent(
          FlashState.empty,
    ...
    test(
      'critical DM not selected adds to openDms + unreadDms, bumps dmPulseNotify',
      () {
        final sel = ChannelSelection({'rf'});
        final s = reduceEvent(
          FlashState.empty,
  ...
  group('reduceEvent — Flashed', () {
    test(
      'selected channel flash bumps flashCounts, flashNotify, uses channel color and flashCount',
      () {
        final sel = ChannelSelection({'rf'});
        final s = reduceEvent(
          FlashState.empty,
    ...
    test(
      'DM flash, peer selected — bumps flashNotify, adds to openDms, no unread',
      () {
    ...
    test(
      'DM flash, peer not selected — adds to openDms and unreadDms, bumps dmPulseNotify',
      () {
        final sel = ChannelSelection({'rf'});
        final s = reduceEvent(
          FlashState.empty,
  ...
  group('flashOutput', () {
    ...
    test('no flashNotify change → playAlert=false, pulse=null', () {
      final out = flashOutput(
        prev,
        prev,
        _defaultSettings.copyWith(audibleAlert: true, flashWholeScreen: true),
      );
    ...
    test(
      'flashWholeScreen=true + flashNotify changed → pulse carries color and count',
      () {
        final out = flashOutput(
          prev,
          next,
  ...
  group('decideFlashCommand — push-event → command, end to end', () {
    ...
    test(
      'a non-critical message with flashOnMessage off produces no command',
      () {
        final sel = ChannelSelection({'rf'});
        final settings = _defaultSettings.copyWith(flashWholeScreen: true);
    ...
    test(
      'a non-critical message with flashOnMessage on produces a command',
      () {
        final sel = ChannelSelection({'rf'});
    ...
    test(
      'a critical message on a non-selected channel updates counts but fires no command',
      () {
        final sel = ChannelSelection({'audio'});
        final settings = _defaultSettings.copyWith(flashWholeScreen: true);
