...
- **Show files** — save and restore named channel layouts; accessible from the folder icon in the sidebar; import/export `.toml` files to share layouts across machines
...
CI (`.github/workflows/ci.yml`) enforces all of these on every PR and push to `main`:
...
# Flutter app
cd patch_app && flutter analyze && flutter test
```
...
- [x] Show files — save / load / delete named channel layouts; sidebar folder icon (not Settings); import/export `.toml` files
...
- [x] Direct messages — private 1:1 peer-to-peer messages (`/patch/dm`), DM threads in the sidebar with unread badges
...
- [x] Graceful shutdown — `/patch/bye` so peers drop promptly instead of waiting out the timeout
- [x] Test suite + CI — Rust engine + Flutter unit/widget tests, enforced by GitHub Actions (fmt · clippy · test · analyze)
