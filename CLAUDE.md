...
| `/patch/presence` heartbeat | Broadcast — per-interface subnet-directed (must reach undiscovered peers) |
...
- Presence/discovery broadcast is **per-interface subnet-directed**, not `255.255.255.255`. `Transport::broadcast`/`broadcast_now` send to `broadcast_targets()` — the subnet broadcast (e.g. `192.168.1.255`) of every usable IPv4 NIC (via the `network-interface` crate's `Addr::broadcast()`, reusing the `SKIP_PREFIXES`/`is_usable_ip` filters), deduped, with `255.255.255.255` as a fallback if none resolve. **Why:** a limited `255.255.255.255` broadcast from a `0.0.0.0`-bound socket only leaves the OS's *primary* interface, so on a machine with a VPN/`utun` or Ethernet alongside Wi-Fi the beacon exited the wrong NIC and discovery was one-way. Subnet-directed targets let the routing table send each copy out the correct interface. When a NIC is pinned (`network_interface`), only that interface's broadcast is used. Computed fresh each heartbeat so it adapts to interfaces appearing/disappearing mid-session. Diagnostic: `cargo test -p patch_core print_broadcast_targets -- --ignored --nocapture`.
