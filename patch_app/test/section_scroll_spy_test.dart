...
void main() {
  ...
  group('SectionScrollSpy', () {
    test('the last section whose top passed the viewport top is active', () {
      ...
      expect(spy.activeFor(sectionTops: [0, 60, 140], viewportTop: 100), 1);
    });

    test('a header within 1px of the top counts as passed (tolerance)', () {
      expect(spy.activeFor(sectionTops: [0, 100.9, 200], viewportTop: 100), 1);
      expect(spy.activeFor(sectionTops: [0, 101.5, 200], viewportTop: 100), 0);
    });
    ...
    test('an unresolved section (null top) is skipped, not a stop', () {
      ...
      // must still be found.
      expect(spy.activeFor(sectionTops: [0, null, 90], viewportTop: 100), 2);
    });
    ...
    test('sections are in document order — the first unpassed section ends '
      ...
      // the viewport top.
      expect(spy.activeFor(sectionTops: [0, 200, 50], viewportTop: 100), 0);
    });

    test('while suppressed (programmatic scroll in flight) every frame is '
        'a no-op', () {
      spy.suppressed = true;
      expect(spy.activeFor(sectionTops: [0, 60], viewportTop: 100), isNull);
      spy.suppressed = false;
      expect(spy.activeFor(sectionTops: [0, 60], viewportTop: 100), 1);
    });
  });
}
