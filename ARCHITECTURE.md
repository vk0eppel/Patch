...
DMs are excluded from the ALL feed and `set_selected_channels`. Non-critical DMs produce a silent unread dot on the peer row (`FlashApplicator.markDmUnread`); critical DMs flash via `FlashApplicator._handlePush` → `decideMessageFlash`. Firing a macro in DM mode sends its text as a DM — routing decided by `resolveMacroTarget` (tap/F-key) and, via `AppState::dm_target`, for a MIDI trigger too (see MIDI above). No ACK/retransmit for DMs (best-effort).
