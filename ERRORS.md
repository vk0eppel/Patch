...
**Every new `ConfigSnapshot` field in Rust must be added to the hand-written `bridge_client.dart::configFromRust()` mapping that builds `AppConfig`.** `getConfig()` returns `Future<AppConfig>`, and the shared `AppStore` is the single consumer — both `home_screen.dart` and `settings_screen.dart` read config-derived values from `store.config`, so wiring the field through `configFromRust` once surfaces it on both screens. That Rust→`AppConfig` hop is still hand-listed. Two tests pin it: a Rust test (`api::tests::config_snapshot_field_set_is_pinned`) pins `ConfigSnapshot`'s serialized field set so adding a Rust field without updating the Dart mapping fails `cargo test`; a Dart sentinel (`test/bridge/config_conversion_test.dart`) constructs a `ConfigSnapshot` with every field at a non-default value and asserts every `AppConfig` field matches, so a wiring miss fails `flutter test`.
